...
export class ImportComponent implements OnInit {
  ...
  privateKey: string;
  invalidPrivateKey: boolean = false;
  ...
  onRestoreWithFile() {
    ...
    if (this.f.password.valid && this.walletKeystore) {
      ...
      try {
        this.cryptoService
          .generateWalletFromKeystore(walletContext.walletKeystore, walletContext.passwordHash, 0)
          .subscribe((w) => {});
        this.walletService.setWalletContext(walletContext);
        this.walletService.generateWalletFromContext();

        setTimeout(() => this.router.navigate(['/wallet']), 100);
      } catch {
  ...
  onRestoreWithMnemonic() {
    ...
    if (this.registerForm.valid && this.mnemonic.valid) {
      ...
      try {
        this.cryptoService
          .generateWalletKeystore(this.mnemonic.value.trim(), passwordHash)
